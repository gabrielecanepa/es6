...
// Every time a page is loaded other HTTP requests are done (for every external resource, scripts, stylesheets, images, apis, fonts, ..)

// HTTP GET Request
// Usually an HTTP request is done with a GET method (called 'the HTTP verb'), containing an URL and an header.
// An header example:
...
// XHR (XMLHttpRequest) is an API in the form of an object to transfer data between a web browser and a web server (use it in the inspector to filter AJAX HTTP requests)
// The response is a JSON (JavaScript Object Notation) response that can be parsed as a JavaScript object, that can then build HTML elements
...
// The best and easier way in ES6 is to use fetch:
fetch(url).then((response) => {
  // Do something once HTTP response is received
});
// Example available at /../ajax.html

// AJAX POST Request (fetch)
// An AJAX fetch POST request require an object to be passed, with 3 properties (method, header, body)
// An example is avaiable at https://kitt.lewagon.com/karr/lectures/front/javascript-102/index.html#/6/7
